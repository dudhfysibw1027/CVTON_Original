...
def add_all_arguments(parser, train):
    ...
    parser.add_argument('--bpgm_id', type=str, default="256_3_5",
                        help="BPGM identification for pretrained weights loading.")
    parser.add_argument('--seg_edit_id', type=str, default="256",
                        help="SEG_EDIT model identification for pretrained weights loading.")
    parser.add_argument('--segmentation', nargs='+', default=["body"],
                        help="Which segmentations to use for conditioning. {body, cloth, densepose}")

    parser.add_argument('--no_seg', action='store_true', default=False,
                        help='whether to train the model without masking clothing')
    parser.add_argument("--no_bg", action='store_true', default=False, help="whether to remove the background in I_m")
    parser.add_argument("--no_bottom", action='store_true', default=False,
                        help="whether to remove the lower part in I_m")
    ...
    if train:
        ...
        parser.add_argument('--add_lpips_loss', action='store_true',
                            help='if specified, add LPIPS feature matching loss')
        ...
        parser.add_argument('--add_d_loss', action="store_true",
                            help="if specified, add segmentation discriminator loss")
        parser.add_argument('--add_cd_loss', action="store_true",
                            help="if specified, add conditional discriminator loss")
        parser.add_argument('--add_pd_loss', action="store_true", help="if specified, add patch discriminator loss")
        parser.add_argument('--add_hd_loss', action="store_true",
                            help="if specified, add human parsing discriminator loss")
        parser.add_argument('--add_bd_loss', action="store_true",
                            help="if specified, add bottom of image discriminator loss")
